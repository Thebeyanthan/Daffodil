...
namespace Daffodil
{
    public partial class Form9 : Form
    {
        ...
        public Form9()
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("select top 1 ID from UKGpayment order by ID desc", conn))
                {
        ...
        {
            // TODO: This line of code loads data into the 'daffodilDataSet8.UKGpayment' table. You can move, or remove it, as needed.
            this.uKGpaymentTableAdapter.Fill(this.daffodilDataSet8.UKGpayment);
            textBox2.Enabled = false;
            textBox3.Enabled = false;
            textBox4.Enabled = false;
            textBox5.Enabled = false;
        ...
        private void checkBox5_CheckedChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Select * from UKGpayment where StudentID = '" + comboBox1.Text + "'", conn))
                    {
        ...
        public void insertData()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionstring))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("insert into UKGpayment values (@id, @stuid, @name, @paydate, @term1, @term2, @term3, @term1Fee, @term2Fee, @term3Fee, @regfee, @bookfee, @other)", conn))
                {
