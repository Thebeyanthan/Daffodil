...
namespace Daffodil
{
    public partial class Form6 : Form
    {
        ...
        {           
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("select top 1 ID from playgroupPayament order by ID desc", conn))
                {
        ...
        {
            // TODO: This line of code loads data into the 'daffodilDataSet6.playgroupPayament' table. You can move, or remove it, as needed.
            this.playgroupPayamentTableAdapter.Fill(this.daffodilDataSet6.playgroupPayament);
        ...
        private void checkBox5_CheckedChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Select * from playgroupPayament where StudentID = '" + comboBox1.Text + "'", conn))
                    {
        ...
        public void insertData()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionstring))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("insert into playgroupPayament values (@id, @stuid, @name, @paydate, @term1, @term2, @term3, @term1Fee, @term2Fee, @term3Fee, @regfee, @bookfee, @other)", conn))
                {
